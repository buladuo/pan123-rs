...
use pan123_sdk::{
    ...
    RetryPolicy, TransferEvent, TransferKind, TransferOptions, UploadDirectoryReport,
    UploadFailureKind, UploadOptions,
};
...
#[derive(Debug, Parser)]
#[command(name = "pan123", about = "鍩轰簬 pan123-sdk 鐨?123 鐩樺懡浠よ宸ュ叿", version)]
struct CliArgs {
    #[command(subcommand)]
...
impl Pan123Cli {
    ...
    fn execute(&mut self, command: Command) -> Result<()> {
        match command {
            Command::Login => {
                if self.client_mut()?.check_token_valid() {
                    println!("{}", "褰撳墠鐧诲綍鐘舵€佷粛鐒舵湁鏁堛€?.green());
                    return Ok(());
                }
                self.client_mut()?.login_by_qrcode()?;
                println!("{}", "鐧诲綍鎴愬姛銆?.green());
            }
            Command::Info => {
                self.ensure_auth()?;
                println!(
            ...
            Command::Mv {
                ...
                println!(
                    ...
                    format!("涓潯鐩埌 {}", target_parent.path).bright_blue()
                );
            }
            Command::Cp {
                target_parent,
            ...
            Command::Refresh { all } => {
                ...
                if all {
                    ...
                    println!(
                        ...
                        "缂撳瓨宸叉竻鐞嗐€?.green(),
                        "宸插垹闄?.bright_white(),
                        format!("{removed} 涓画浼犲厓鏁版嵁鏂囦欢").bright_yellow()
                    );
                } else {
                    println!("{}", "琛ュ叏缂撳瓨宸叉竻鐞嗐€?.green());
                }
            }
            Command::Clear => {
                clear_screen()?;
    ...
    fn run_interactive(&mut self) -> Result<()> {
        println!("{}", "123pan 浜や簰寮忕粓绔?.bright_cyan().bold());
        println!(
            "{}",
            "鎸?Tab 鍙ˉ鍏ㄥ懡浠ゅ拰杩滅璺緞銆傝緭鍏?help 鏌ョ湅鍛戒护锛岃緭鍏?exit 閫€鍑恒€俓n"
                .dimmed()
        );
    ...
    fn print_help(&self) {
        println!(
            ...
            "download <REF>... [-d 目录] [--retries N]".bright_magenta(),
            ...
            "status|stat <REF> [--json], find <QUERY> [-p REF], refresh [--all], clear, clear".bright_magenta(),
            "REF 鍙互鏄?file_id銆乣id:123`銆佸綋鍓嶇洰褰曚笅鐨勫悕瀛楋紝鎴栬€?`/docs/a.txt`銆乣../tmp` 杩欐牱鐨勮矾寰勩€俙find` 鏀寔鏅€氬寘鍚€侀€氶厤绗︼紙`*.zip`锛夊拰鍩虹姝ｅ垯锛坄re:^code`锛夈€?.dimmed()
        );
    }
    ...
    fn ensure_auth(&mut self) -> Result<()> {
        ...
        println!("{}", "闇€瑕佺櫥褰曪紝姝ｅ湪鍚姩浜岀淮鐮佺櫥褰?..".yellow());
        self.client_mut()?.login_by_qrcode()
    }
    ...
    fn print_upload_report(&self, report: &UploadDirectoryReport) {
        println!(
            ...
            format!("{} 成功", report.uploaded_count()).green(),
            format!("{} 失败", report.失败_count()).red(),
    ...
    fn print_find_results(&self, results: &[FileInfoWithPath]) {
        ...
        for result in results {
            let kind = if result.item.is_dir() {
                "[D]".bright_blue().to_string()
            } else {
                "[F]".bright_green().to_string()
            };
            println!(
                "{} {} {} {}{}{}",
                kind,
...
impl FindMatcher {
    fn matches(&self, name: &str) -> bool {
        match self {
            FindMatcher::Exact(value) => name == value,
            FindMatcher::Contains(value) => name.to_lowercase().contains(value),
            FindMatcher::Wildcard(pattern) => wildcard_match(pattern, name),
            FindMatcher::Regex(pattern) => basic_regex_match(pattern, name),
        }
    }
}
...
impl Completer for ShellHelper {
    ...
    fn complete(
        ...
        let candidates = if tokens.is_empty() {
            ...
            match command {
                ...
                "cd" => self
                    ...
                    .map(|mut s| s.remote_candidates(&token, true))
                    .unwrap_or_default(),
                "ls" | "tree" | "mkdir" if is_option_value(prev, &["-p", "--parent"]) => self
                    .state
                    .lock()
                    .ok()
                    .map(|mut s| s.remote_candidates(&token, true))
        ...
        let candidates = promote_common_prefix(&token, candidates);
        Ok((start, candidates))
    }
}
...
fn build_editor() -> Result<Editor<ShellHelper, DefaultHistory>> {
    let config = RustyConfig::builder()
        .completion_type(Completion类型：:List)
        .build();
...
fn progress_style(has_total: bool) -> ProgressStyle {
    if has_total {
        ProgressStyle::with_template(
            "{prefix:.bold} [{bar:32.cyan/blue}] {bytes}/{total_bytes} {binary_bytes_per_sec} ETA {eta} {msg}",
        )
        .unwrap()
    } else {
        ProgressStyle::with_template(
...
fn validate_basic_regex(pattern: &str) -> Result<()> {
    ...
    if chars.first() == Some(&'*') {
        return Err(Pan123Error::Operation(
            "invalid regex: '*' cannot appear at the start".into(),
        ));
    }
    for window in chars.windows(2) {
        if window[0] == '*' && window[1] == '*' {
            return Err(Pan123Error::Operation(
                "invalid regex: consecutive '*' is not supported".into(),
            ));
        }
    }
    Ok(())
